...
function dice() {
  const ranNumber = Math.ceil(Math.random() * 6);
  return ranNumber;
}
...
function splitMe() {
  const word = "Essere o non essere";
  const dividedWord = word.split(" ");
  const maxWord = dividedWord.map(
...
function deleteOne(str, Statement) {
  if (Statement) {
...
console.log(deleteOne("Musica", false));
console.log(deleteOne("Musica", true));
...
function onlyLetters() {
  const numberWords = "Ho 7 libri e 2 penne";
  const noNumberWords = numberWords.replace(/\d+/g, "");
  return noNumberWords;
}
...
function isThisAnEmail() {
  const email = prompt("Inserisci un indirizzo email");
  if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    return true;
  } else {
    return false;
...
function sumAllTheYears(movies) {
  return movies.reduce((total, movie) => {
    const year = parseInt(movie.Year, 10);
    return total + (isNaN(year) ? 0 : year);
  }, 0);
}
...
function removeIndex(index, movies) {
  ...
  if (index >= 0 && index < newMovies.length) {
...
function redBackgroundLinks() {
  const links = document.querySelectorAll("a");

  // Itera su ogni link
  links.forEach((link) => {
    // Imposta il background color a rosso
